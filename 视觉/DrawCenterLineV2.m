...
% 找中心线算法，输入赛道类型class_Name和逆透视二值化图片image，输出绘制有左线、右线、中心线的逆透视二值化图片image_withLine
    ...
    if strcmp(class_Name,'左弯') || strcmp(class_Name,'右弯') || strcmp(class_Name,'未知')
        ...
        for i=1:search_Lines%从最后一行开始逐行扫描，一共扫描search_Lines行
            ...
            elseif Col_Right(i)~=-2 && Col_Left(i)==-2 %左线非法，右线合法
                if Col_Center(i-1)~=-2 %如果上一次中心线合法
                    Col_Center(i) = filter*Col_Center(i-1)+(1-filter)*(Col_Right(i)-road_width/2.0); %根据上一次的中心线、假定的这一次中心线（保持上一次和右线的距离），用滤波计算这次的中心线
                else %如果上一次中心线非法
                    Col_Center(i) = filter*ceil(size(image,2)/2)+(1-filter)*(Col_Right(i)-road_width/2.0); 
                end
            elseif Col_Right(i)==-2 && Col_Left(i)~=-2 %左线合法，右线非法，与上文类似
                if Col_Center(i-1)~=-2 && Col_Left(i-1)~=-2 
                    Col_Center(i) = filter*Col_Center(i-1)+(1-filter)*(Col_Left(i)+road_width/2.0);
                else
                    Col_Center(i) = filter*ceil(size(image,2)/2)+(1-filter)*(Col_Left(i)+road_width/2.0);
                end
            else %左线非法，右线非法
    ...
    elseif strcmp(class_Name,'三岔路口') || strcmp(class_Name,'右环岛')
        road_width = 20;
        filter = 0;%滤波系数，正数时是低通滤波，负数时相当于高通滤波
        search_Lines = size(image,1);%一共要扫描多少行，最大是图片宽
        ...
        for i=1:search_Lines%从最后一行开始逐行扫描，一共扫描search_Lines行
            ...
            if image(start_Row,start_Col(1)) == 1 %如果左线发现1区域（道路）  
                Col_Left(i) = start_Col(1);%发现1区域就保持右线往左road_width
            else%如果左线发现0区域（背景）   
                cnt_temp = 0;
                while image(start_Row,start_Col(1)) == 0 && start_Col(1)<=size(image,2)-1
                    start_Col(1) = start_Col(1) + 1;
