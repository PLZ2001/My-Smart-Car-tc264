...
uint8 UART_EN = FALSE;//用于表示上位机功能是否开启
...
void UART(enum UARTstate state)
{
    if (state == Send)
    {
        ...
        if (UART_Flag_TX == TRUE || UART_Flag_NO_IMAGE == TRUE)
        {
            ...
            //发送舵机参数，数据头00-FF-05-01，数据长度5字节，数据尾00-FF-05-02
            UART_Steering();
            ...
            //发送gyro_y参数，数据头00-FF-14-01，数据长度4字节，数据尾00-FF-14-02
            UART_ICM();

            //发送总速度目标参数，数据头00-FF-15-01，数据长度2字节，数据尾00-FF-15-02
            UART_Speed_Mode();
...
//            }
    ...
    else if (state == Emergency_Send)
    {
        ...
        if (UART_Flag_TX == TRUE)
        {
            ...
            //发送总速度目标参数，数据头00-FF-15-01，数据长度12字节，数据尾00-FF-15-02
            UART_Speed_Mode();
