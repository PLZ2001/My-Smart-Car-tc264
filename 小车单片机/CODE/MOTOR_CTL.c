...
void Differential_Motor(void)
{
    ...
    if (speed_Mode == High_Mode)
    {
        if (steering_Error>=0)
        {
            speed_Target2 = speed_Target + speed_Target*OuterSide_Ratio*Differential_Ratio*Limit(steering_Target*fabsf(steering_Target)/60.0f,20.0f)*4.0f/1062.5f;//左轮目标速度（m/s）
            speed_Target1 = speed_Target - 1.2f*speed_Target*InnerSide_Ratio*Differential_Ratio*Limit(steering_Target*fabsf(steering_Target)/60.0f,20.0f)*4.0f/1062.5f;
    ...
    //        speed_Target1 = speed_Target - 0.9*steering_Error/600*(speed_Target+Differential_Ratio)/(1.2f+1);
        ...
        {
            speed_Target1 = speed_Target - speed_Target*OuterSide_Ratio*Differential_Ratio*Limit(steering_Target*fabsf(steering_Target)/60.0f,20.0f)*4.0f/1062.5f;//右轮目标速度（m/s）
            speed_Target2 = speed_Target + 1.2f*speed_Target*InnerSide_Ratio*Differential_Ratio*Limit(steering_Target*fabsf(steering_Target)/60.0f,20.0f)*4.0f/1062.5f;
    ...
    else
    {
        if (steering_Error>=0)
        {
            speed_Target2 = speed_Target + speed_Target*OuterSide_Ratio*Differential_Ratio*steering_Target*4.0f/1062.5f;//左轮目标速度（m/s）
            speed_Target1 = speed_Target - /*0.9f**/1.2f*speed_Target*InnerSide_Ratio*Differential_Ratio*steering_Target*4.0f/1062.5f;
    //        speed_Target2 = speed_Target + steering_Error/600*(speed_Target+Differential_Ratio)/(1.2f+1);//左轮目标速度（m/s）
    //        speed_Target1 = speed_Target - 0.9*steering_Error/600*(speed_Target+Differential_Ratio)/(1.2f+1);
        }
        else
        {
            speed_Target1 = speed_Target - speed_Target*OuterSide_Ratio*Differential_Ratio*steering_Target*4.0f/1062.5f;//右轮目标速度（m/s）
            speed_Target2 = speed_Target + 1.2f*speed_Target*InnerSide_Ratio*Differential_Ratio*steering_Target*4.0f/1062.5f;
    //        speed_Target1 = speed_Target - steering_Error/600*(speed_Target+Differential_Ratio)/(1.2f+1);//右轮目标速度（m/s）
    //        speed_Target2 = speed_Target + 0.9*steering_Error/600*(speed_Target+Differential_Ratio)/(1.2f+1);;
        }
    }
...
void Update_Speed_Mode(void)
{
    switch (speed_Mode)
    {
        case Lowest_Mode:
        {
            ...
            speed_Target_Highest = 3.7f+0.5f-0.3f;//即3.3
            SightForward_Highest = 0.35f;
            OuterSide_Ratio_Highest = 0.9f;
            InnerSide_Ratio_Highest = 1.1f;
            Steering_PID_Highest[0]=0.13f;Steering_PID_Highest[1]=0;Steering_PID_Highest[2]=1.0f;


            speed_Target_High = 3.2f+0.2f-0.3f;//即2.8
            SightForward_High = 0.35f;
            OuterSide_Ratio_High = 0.9f;
            InnerSide_Ratio_High = 1.1f;//1.15f;
            Steering_PID_High[0]=0.13f;Steering_PID_High[1]=0;Steering_PID_High[2]=1.0f;


            speed_Target_Low = 3.1f-0.3f;//即2.7
            SightForward_Low = 0.35f;
            OuterSide_Ratio_Low = 0.9f;
            InnerSide_Ratio_Low = 1.1f;//1.25;
            Steering_PID_Low[0]=0.14f;Steering_PID_Low[1]=0;Steering_PID_Low[2]=1.0f;


            speed_Target_Lowest = 3.0f-0.3f;//即2.6
            SightForward_Lowest = 0.35f;
            OuterSide_Ratio_Lowest = 0.9f;
            InnerSide_Ratio_Lowest = 1.1f;
            Steering_PID_Lowest[0]=0.14f;Steering_PID_Lowest[1]=0;Steering_PID_Lowest[2]=1.0f;

            speed_Target_Lowest_ForT = 2.4f;//即2.1
            SightForward_Lowest_ForT = 0.35f;
            OuterSide_Ratio_Lowest_ForT = 0.9f;
            InnerSide_Ratio_Lowest_ForT = 1.1f;
            //Steering_PID_Lowest_ForT[0]=2.50f;Steering_PID_Lowest_ForT[1]=0;Steering_PID_Lowest_ForT[2]=0.10f;
            Steering_PID_Lowest_ForT[0]=0.08f;Steering_PID_Lowest_ForT[1]=0;Steering_PID_Lowest_ForT[2]=0.8f;
        ...
        case Low_Mode:
        {
            ...
            speed_Target_Highest = 3.7f+0.5f+0.8f;//即3.3
            SightForward_Highest = 0.35f;
            OuterSide_Ratio_Highest = 0.9f;
            InnerSide_Ratio_Highest = 1.1f;
            ...
            speed_Target_High = 3.2f+0.2f+0.4f;//即2.8
            SightForward_High = 0.35f;
            OuterSide_Ratio_High = 0.9f;
            InnerSide_Ratio_High = 1.1f;//1.15f;
...
//            break;
            ...
            speed_Target_Highest = 5.0f;//即4.4
            ...
            speed_Target_High = 3.6f+0.4f;//即3.3
            SightForward_High = 0.43f;
            OuterSide_Ratio_High = 2.0f;
            InnerSide_Ratio_High = 1.65f;//1.15f;
            Steering_PID_High[0]=0.145f;Steering_PID_High[1]=0;Steering_PID_High[2]=1.1f;


            speed_Target_Low = 3.6f+0.1f;//即3.2
            SightForward_Low = 0.43f;
            OuterSide_Ratio_Low = 2.0f;
            InnerSide_Ratio_Low = 1.9f;//1.25;
            Steering_PID_Low[0]=0.145f;Steering_PID_Low[1]=0;Steering_PID_Low[2]=1.1f;


            speed_Target_Lowest = 3.5f;//即3.1
            SightForward_Lowest = 0.43f;
            OuterSide_Ratio_Lowest = 2.0f;
            InnerSide_Ratio_Lowest = 2.1f;
            Steering_PID_Lowest[0]=0.145f;Steering_PID_Lowest[1]=0;Steering_PID_Lowest[2]=1.1f;
        ...
        case Highest_Mode:
        {
            ...
            speed_Target_Highest = 3.7f+0.5f+0.2f;//即3.3
            SightForward_Highest = 0.35f;
            OuterSide_Ratio_Highest = 0.9f;
            InnerSide_Ratio_Highest = 1.1f;
            Steering_PID_Highest[0]=0.14f;Steering_PID_Highest[1]=0;Steering_PID_Highest[2]=1.0f;
            ...
            speed_Target_High = 3.2f+0.2f+0.2f;//即2.8
            SightForward_High = 0.35f;
            OuterSide_Ratio_High = 0.9f;
            InnerSide_Ratio_High = 1.1f;//1.15f;
            Steering_PID_High[0]=0.14f;Steering_PID_High[1]=0;Steering_PID_High[2]=1.0f;
            ...
            speed_Target_Low = 3.1f+0.2f;//即2.7
            SightForward_Low = 0.35f;
            OuterSide_Ratio_Low = 0.9f;
            InnerSide_Ratio_Low = 1.1f;//1.25;
            Steering_PID_Low[0]=0.15f;Steering_PID_Low[1]=0;Steering_PID_Low[2]=1.0f;
            ...
            speed_Target_Lowest = 3.0f+0.1f;//即2.6
            SightForward_Lowest = 0.35f;
            OuterSide_Ratio_Lowest = 0.9f;
            InnerSide_Ratio_Lowest = 1.1f;
            Steering_PID_Lowest[0]=0.15f;Steering_PID_Lowest[1]=0;Steering_PID_Lowest[2]=1.0f;
